...
export default {    
    ...
    mounted : function() {
      ...
      this.$eventBus.$on("addSubmit", (contact) => {
      ...
      this.$eventBus.$on("updateSubmit", (contact) => {
          this.currentView = null;
          this.updateContact(contact);
      });
      this.$eventBus.$on("addContactForm", () => {
          this.currentView = 'addContact';
      });
      this.$eventBus.$on("editContactForm", (no) => {
          this.fetchContactOne(no)
          this.currentView = 'updateContact';
      });
      this.$eventBus.$on("deleteContact", (no) => {
          this.deleteContact(no);
      });
      this.$eventBus.$on("editPhoto", (no) => {
          this.fetchContactOne(no)
          this.currentView = 'updatePhoto';
      });
      this.$eventBus.$on("updatePhoto", (no, file) => {
          if (typeof file !=='undefined') {
              this.updatePhoto(no, file);
          }
          this.currentView = null;
      });
      this.$eventBus.$on("pageChanged", (page)=> {
    ...
    methods : {
      ...
      fetchContactOne : function(no) {
          let contact = this.contactlist.contacts.find((c)=>c.no === no);
